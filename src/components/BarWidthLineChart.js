...
export default function BarWidthLineChart(props) {
  const { reciveBarList } = props;
  ...
  const { reciveLegend } = props;
  const { reciveLineInfo } = props;
  const { reciveCheckInfo } = props;
  // 기본
  const legend = reciveLegend
    ? reciveLegend
    : ['읽은 시간', '비슷한 사용자가 읽은 시간'];

  const lineInfo = reciveLineInfo
    ? reciveLineInfo
    : [
        {
          source: 2,
  ...
  const checkInfo = reciveCheckInfo ? reciveCheckInfo : [2, 4];

  // 화면 랜더링 후 좌표에 대한 위치를 구함
  useEffect(() => {
    const calcForOffsetId = barList
      .flatMap((val) => val.percentList)
      .map((item) => item.id);
    const caclOffset = {};
  ...
  return (
    <div className="multi-bar-wrap">
      <svg className="line-con">
        ...
        {lineCalcOk &&
          checkInfo.map((checkId) => {
            return (
              <g
                transform={`translate(${offsetInfo[checkId].x - 8},${
                  offsetInfo[checkId].y - 26
                })`}
              >
                <path
      ...
      <div className="bar-grap-container">
        {barList.map((item) => {
          return (
            <div
              className="multi-bar-con"
              style={{ width: `calc(100% / ${barList.length}` }}
            >
        ...
        {barList.map((item) => {
          return (
            <div
              className={
                item.disabled
                  ? 'bar-title-con multi-bar-title disabled'
                  : 'bar-title-con multi-bar-title'
              }
            >
              <div className="bar-txt">{item.title}</div>
            </div>
          );
        })}
      </div>
